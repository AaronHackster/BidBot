 ...
 if(isset($_REQUEST['r']))
 {
	...
	$row=mysqli_fetch_array($sq,MYSQLI_ASSOC);
	 if($row["status"]=="NOT OVER")
	 {
		...
		if($rowb["BuyerID"]==$_SESSION['login_user_id'])
		{
			
			
			header("location: Bid_won.php?q=".$oid);
		}
		}
	 }
 }
...
elseif (isset($_REQUEST['c'])) 
 ...
 $productCount=mysqli_num_rows($sql);$i=0;
 if($productCount>0)
 {
	 ...
	 while($row=mysqli_fetch_array($sql,MYSQLI_ASSOC))
	 {
		...
		$pid=$row["ProductID"]; 
		$enddate=$row["EndTime"];//product_bidded_details.php
  ...
  <hr>
    <div class="productprice"><div id="'.$pid.'p" class="pricetext">Rs '.$price.'</div>
	<hr>
...
//<script> initializeClock("'.$pid.'","'.$enddate.'"); </script>
  if(isset($_REQUEST['q']))	    
	 $sqlsold=mysqli_query($conn,"SELECT * from products,auction where products.ProductID=auction.ProductID and auction.status='OVER' and Pname like '%".$_REQUEST['q']."%' ORDER BY products.ProductID" );		
elseif (isset($_REQUEST['c'])) 
	 $sqlsold=mysqli_query($conn,"SELECT * from products,auction where products.ProductID=auction.ProductID and auction.status='OVER' and CategoryID=".$_REQUEST['c']." ORDER BY products.ProductID" );		
 else
 $sqlsold=mysqli_query($conn,"SELECT * from products,auction where products.ProductID=auction.ProductID and auction.status='OVER' ORDER BY products.ProductID");
	
	$prodc2=mysqli_num_rows($sqlsold);
	
		 while($row=mysqli_fetch_array($sqlsold,MYSQLI_ASSOC))
	 {
		 		$pname=$row["Pname"];
		$price=$row["CurrentPrice"];
		$pid=$row["ProductID"];
  ...
  <hr>
    <div class="productprice"><div id="'.$pid.'p" class="pricetext">Rs '.$price.'</div>
	<hr>
	<div class=""><button class="btn btn-info btn-sm" type="submit" role="button" onclick="details_only('.$pid.');" >View Details</button></div>
	</div>
	</div>
</div>
';
...
<head>
	...
	function changeprice(a)
	{
		...
		for(var i=0;i<a.length;i++)
		{
			...
			//console.log(i);
			if(a[i]!=undefined && a[i]!=" "&&a[i]!=""&&a[i+1]!=undefined && a[i+1]!=" " && a[i+1]!=null &a[i]!=null  && a[i+2]!=null &a[i+2]!=undefined)
			{
				a1=parseInt(a[i]);
				a2=parseInt(a[i+1]);
				console.log(a);
				var c=document.getElementById(a2+"p");
				c.innerHTML="Rs "+a1;
				initializeClock(a2,a[i+2]+" "+a[i+3]);
	...
	function createlink()
{
  ...
  //console.log(t.value);
  xhttp.open("GET", "updateindex.php?q="+t.value, true);
  xhttp.send();
}
...
<body style="background-color: rgb(241, 241, 241);">
	...
	function makebid()
	{
		 ...
		 <input type="hidden" name="some" id="secret" value='<?php 
		if(isset($_REQUEST['q']))
			echo "@".($_REQUEST['q']);
	...
	<section id="slider"><!--slider-->
		...
		-->   <div class="container" style="border-radius:5px;margin-left:15%;">
			<div class="row">
				<div class="col-sm-12">
					<div id="slider-carousel" class="carousel slide" data-ride="carousel"  >
						<ol class="carousel-indicators" style="margin-left:-40%; ">
							<li data-target="#slider-carousel" data-slide-to="0" class="active"></li>
							<li data-target="#slider-carousel" data-slide-to="1"></li>
							<li data-target="#slider-carousel" data-slide-to="2"></li>
						</ol>
						
						<div class="carousel-inner" style="border-radius:1px solid black">
